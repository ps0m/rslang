import { AMOUNT_VARIANTS_OF_AUDIO_GAMES } from "../constants/constatnts";
import { getCurrentDate, getMaxSeries, shuffleArray } from "../helpers/helpers";
import { IAuth, IContentForAudio, ICustomStat, IDifficulty, IElemOptionalProgress, IGameDailyStatisticKey, IPropertyWord, IStatistic, IWords } from "../types/types";
import { createUserWord, getAllUserWords, getUserStatistics, getUserWord, updateUserStatistics, updateUserWord } from "./API/API";

// const amountLearned = 0



export const getOrderRandomWords = (initWords: IWords[], currWords: IWords[]): IContentForAudio | null => {
...
const divisionCurrentStatistic = async ({ isAuth, currentStatistic }: IDivisionCurrentStatistic) => {
  try {
...
const createNewStatistic = ({ currentStatistic, oldStatistics, newUserWords, currentGame, amountLearned }: ICreateNewStatistic) => {
  ...
  const sprintIfCurrentDateContent = currentGame === 'audio'
    ? {
  ...
  const audioIfCurrentDateContent = currentGame === 'sprint'
    ? {
      ...oldStatistics.optional.daily[notCurrentGame]
    }
    : {
    ...
    optional: {
      ...
      longTerm: {
        ...
        [oldStatistics.optional.daily.date]: {
          newLearnedWords: oldStatistics.optional.longTerm[oldStatistics.optional.daily.date].newLearnedWords + amountNewWords,
          totalLearnedWords: oldStatistics.optional.longTerm[oldStatistics.optional.daily.date].totalLearnedWords + amountLearned
        }
      }
    }
  }
  ...
  const sprintIfNewDateContent = currentGame === 'audio'
    ? {
  ...
  const audioIfNewDateContent = currentGame === 'sprint'
    ? {
  ...
  const statIfNewDate: IStatistic = {
    ...
    optional: {
      ...
      longTerm: {
        ...
        [getCurrentDate()]: {
          newLearnedWords: 0,
          totalLearnedWords: oldStatistics.optional.longTerm[oldStatistics.optional.daily.date].totalLearnedWords
        }
      }
    }
  }

  const newStat: IStatistic = oldStatistics.optional.daily.date === getCurrentDate()
    ? statIfCurrentDate
    : statIfNewDate
...
export const updateAfterGame = async ({ isAuth, currentStatistic, currentGame }: IUpdateAfterGame) => {
  try {
    if (isAuth !== null && currentStatistic !== null) {
      ...
      const responseUpdateUserWords: Promise<{ last: IPropertyWord; updated: IPropertyWord; } | undefined>[] = oldUserWords.map(wordOfStat => {
      ...
      for await (const iterator of responseUpdateUserWords) {
        ...
        if (iterator.last.optional.learned === false && iterator.updated.optional.learned === true) {
          amountLearned++
        }

        if (iterator.last.optional.learned === true && iterator.updated.optional.learned === false) {
          amountLearned--
        }
      }
      ...
      const newStatistic: IStatistic = createNewStatistic({ currentStatistic, oldStatistics, newUserWords, currentGame, amountLearned });
